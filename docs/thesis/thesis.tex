...
absolute discriminating power, while the modern attention model
saturates only slowly with $N_{\text{train}}$ and is the
worst-calibrated of the four before temperature scaling.  All
results are reproducible on a single CPU laptop in under one hour
using the open-source pipeline released with this thesis.
...
\paragraph{Findings.}
\begin{itemize}
    \item The BDT is by far the most data-efficient: it gains less
          than half a percent of AUC between $5$\,k and $50$\,k
          jets.  Engineered features short-circuit much of what the
          deep models have to learn from scratch.
    \item ParticleNet and the Particle Transformer continue to gain
          measurably with $N_{\text{train}}$ all the way to
          $5\!\times\!10^4$ jets.  Their slope on the rejection plot
          is positive at the largest size we tested, consistent
          with the literature observation that they only saturate
          near $10^6$ jets~\cite{partformer}.
    \item The CNN's curve is intermediate: it benefits from extra
          data but plateaus earlier than the permutation-equivariant
          architectures.
\end{itemize}
...
\paragraph{Findings.}
\begin{itemize}
    \item The BDT is essentially well-calibrated already, with an
          ECE below $0.01$.  This reflects the fact that XGBoost
          minimises a proper scoring rule (log-loss) directly on the
          probability.
    \item The neural models are systematically more over-confident,
          and the Particle Transformer is the worst-calibrated of
          the four.  This is consistent with the original
          observation of Guo \emph{et al.}~\cite{guo2017}: deeper
          models tend to over-confidence.
    \item A single scalar $T$, fit on the validation set, brings
          all models within ECE\,$\lesssim\!0.01$ on the test set,
          at zero training-data cost.
\end{itemize}
...
score through a temperature rescaling fit on a held-out sample.
The cost is one scalar parameter; the benefit is bringing the ECE
below the $1\%$ level on which most likelihood-based limit setters
would otherwise systematically mis-cover.
...
classical BDT --- which is essentially data-efficient at the cost
of a couple of percent of AUC --- and the modern permutation-
equivariant models, which continue to gain measurably with extra
data up to and beyond the budget of this study.

The calibration study shows that the Particle Transformer,
although the highest-AUC model considered, is also the
worst-calibrated, but that a single-parameter temperature
rescaling on the validation set restores calibration essentially
for free.
